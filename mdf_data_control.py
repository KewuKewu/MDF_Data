...
def export_all_cards_to_json(conn, json_file="allCards.json"):
	cursor = conn.cursor()
	...
	for row in cursor.fetchall():
		card = dict(zip(col_names, row))
		# 解析 JSON 字段
		for key in ['related_tags', 'acquisition', 'card_features']:
			if card.get(key):
				try:
	...
	print(f"已导出 {len(cards)} 张卡牌到 {json_file}")
